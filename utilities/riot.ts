...
export const getSummonerByName = async (name: string): Promise<Summoner> => {
  ...
  const response = await fetch(
    `${RIOT_ORIGIN}/tft/summoner/v1/summoners/by-name/${name}`,
    {
  ...
  throw new Error(
    `Error getting summoner data. Riot returned a ${response.status} code.`
  );
};
...
export const getSummonerLeagueEntryBySummonerId = async (
  ...
  const response = await fetch(
    `${RIOT_ORIGIN}/tft/league/v1/entries/by-summoner/${summonerId}`,
    {
      headers: { "X-Riot-Token": Environment.riotToken },
    }
  );
  ...
  throw new Error(
    `Error getting league entry data. Riot returned a ${response.status} code.`
  );
};

export const sortLeagueEntriesByRank = (
  entries: LeagueEntry[]
): LeagueEntry[] => {
...
export const convertTierAndRankToNumber = (
  tier: string,
  rank: string,
  leaguePoints: number
): number => {
