...
export const getLastRefreshTime = async (): Promise<Date> => {
  ...
  const timestamp = snapshot.docs[0].data()
    .lastRefreshTime as firestore.Timestamp;
...
export const updateLastRefreshTimeToNow = async (): Promise<void> => {
  ...
  await db
    ...
    .set({ lastRefreshTime: firestore.Timestamp.now() });
};

export const updateSummonerIdForLeaderboardEntry = async (
  id: string,
  summonerId: string
): Promise<void> => {
...
export const updateLastLeagueEntryForLeaderboardEntry = async (
  id: string,
  lastLeagueEntry: any
): Promise<void> => {
  db.collection("leaderboard_entries").doc(id).update({
